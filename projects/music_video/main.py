    ...
    for video in pending_videos:
        ...
        try:
            ...
            if not os.path.exists(idea_file):
                ...
                # Pass avoid_genres here, and skip image generation if using Veo (will be done in video loop step)
                idea = idea_generator.generate_idea_to_file(
                    idea_file, 
                    cover_image, 
                    existing_titles=existing_titles, 
                    avoid_genres=avoid_genres, 
                    dev_mode=args.dev,
                    skip_image_generation=args.enable_veo
                )
                if not idea:
            ...
            # Step 1b: Generate Thumbnail MOVED to after Step 2
            # because Step 2 might generate the base image if using Veo
            ...
            # Step 1b: Generate Thumbnail (Moved here)
            ...
            # Step 2b: Create Seamless Loop (Legacy "Cut, Swap, Fade" technique)
            # Now applied to Veo videos as well to ensure seamlessness
            if not os.path.exists(seamless_loop):
                ...
                # If file exists, we use it. 
                pass
            
            # Step 2c: Add Title Overlay to Loop (for intro only)
            video_loop_with_title = os.path.join(assets_dir, "visuals_loop_with_title.mp4")
            if not os.path.exists(video_loop_with_title):
                logging.info("Step 2c: Adding Title Overlay to 8-second loop...")
                from projects.music_video.title_overlay import add_title_overlay
                try:
                    success = add_title_overlay(idea_file, seamless_loop, video_loop_with_title)
                    if success:
                        logging.info(f"Title overlay added to loop")
                    else:
                        logging.warning("Failed to add title overlay, intro will correspond to loop")
                except Exception as e:
                    logging.warning(f"Error adding title overlay: {e}. Intro will correspond to loop.")
            else:
                logging.info("Loop with title already exists.")
            ...
            if not os.path.exists(final_video):
                ...
                # Check if we have a titled intro video
                intro_video = None
                if os.path.exists(video_loop_with_title):
                    intro_video = video_loop_with_title
                    
                final_assembler.assemble_final_video(
                    final_audio, 
                    seamless_loop, 
                    final_video, 
